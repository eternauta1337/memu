// Importa el histórico de WhatsApp desde el store de whatsmeow (`wacli.db`) a la DB de Memu.
//
...
// `messages` de Memu, para que el cerebro arranque con contexto y no solo con lo nuevo.
//
...
import type { ChatKind, MemuMessage } from "./ingest.ts";
...
function rowToMemu(row: WacliDbRow, ownPhone: string | null): MemuMessage {
...
/** Importa el histórico de wacli.db → memu.db de un usuario. Reusable: la corre el CLI (dueño) y
 *  el orquestador al hot-add de un usuario nuevo. Excluye archivados (salvo MEMU_INGEST_ARCHIVED). */
export async function importHistoryForUser(userId: string): Promise<{ inserted: number; total: number }> {
  ...
  // Chats archivados: no se importan (salvo MEMU_INGEST_ARCHIVED=1).
  ...
  const importAll = store.db.transaction((batch: WacliDbRow[]) => {
    for (const row of batch) {
      if (store.save(rowToMemu(row, ownPhone))) inserted++;
    }
  });
  importAll(rows);
  src.close();
...
async function main(): Promise<void> {
  ...
  console.log(dim(`total en Memu: ${before} → ${store.count()}`));
}
